...
if [ "$NOW" -ge 45 ] && [ "$NOW" -le 150 ]; then
  # الحارس بالساعة مش بالحالة — بيمنع بناءين مع بعض (2GB × 2 على 3.9GB رام
  # = الـ OOM killer بيقتل عمليات عشوائي وممكن يوقّع الديمو).
  ...
  if [ "${SKIP_TIME_CHECK:-0}" = "1" ]; then
   ...
   الديمو بيصفّر ١:٠٠ وبيبني ١:٣٠، وبناءين مع بعض على 3.9GB رام هيوقّعوا الاتنين.
   استنى لحد 02:30، أو لو اتأكدت إن الديمو مش بيبني: SKIP_TIME_CHECK=1 bash \$0"
  fi
else
