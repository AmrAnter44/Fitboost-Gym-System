...
export default function PhysiotherapyCommissionPage() {
  ...
  const toggleSeparateCoachTarget = async (next: boolean) => {
    ...
    setUseSeparateCoachTarget(next)
    try {
      ...
      setUseSeparateCoachTarget(prev)
  ...
  const calculatePercentage = (income: number): number => {
    //  لما التارجت المنفصل ON، النسبة بتيجي يدوياً
  ...
  const handleConfirmPayroll = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(t('physiotherapy.commission.payrollFail'))
      }
    } catch {
      toast.error(t('physiotherapy.commission.payrollConnectionError'))
    } finally {
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6">
          ...
          {loading ? (
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {/* جدول النسب - Dynamic flex tiers (مخفي لما التارجت المنفصل ON) */}
                  ...
                  <div className="bg-gradient-to-br from-primary-50 to-primary-50 dark:from-primary-900/50 dark:to-primary-900/50 ring-1 ring-primary-200 dark:ring-primary-700 rounded-xl p-5 dark:border-gray-600 dark:bg-gray-700 dark:text-primary-contrast">
                    ...
                    <div className="space-y-2 text-sm">
                      {Array.from({ length: n }).map((_, i) => {
                        let label: string
                        if (i === 0) {
                          label = `${t('physiotherapy.commission.lessThanAmount', { amount: limits[0].toLocaleString(localeString) })} ${t('physiotherapy.commission.egp')}`
                        } else if (i === n - 1) {
                          label = `${t('physiotherapy.commission.orMoreAmount', { amount: limits[n - 2].toLocaleString(localeString) })} ${t('physiotherapy.commission.egp')}`
                        } else {
                          label = `${limits[i - 1].toLocaleString(localeString)} - ${(limits[i] - 1).toLocaleString(localeString)} ${t('physiotherapy.commission.egp')}`
                        }
                        return (
              ...
              {/*  Input النسبة اليدوية — تظهر بس لما التارجت المنفصل ON */}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6">
          ...
          <div className="flex flex-col items-center justify-center h-full py-8 sm:py-12">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {(() => {
                const staff = coaches.find(c => c.name === result.therapistName)
                  ...
                  <div>
                    ...
                    {lastDate && (
                      <p className="text-xs text-center text-gray-500 dark:text-gray-400 mt-1">
                        {t('physiotherapy.commission.lastPayroll')}: {new Date(lastDate).toLocaleDateString(localeString, { day: 'numeric', month: 'short', year: 'numeric' })}
                      </p>
                    )}
                  </div>
                )
              ...
              <div className="space-y-4">
                {sessionCommissions.map((coach, index) => (
                  <div key={index} className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-md ring-1 ring-green-200 dark:ring-green-700">
                    ...
                    {(() => {
                      const staff = coaches.find(c => c.name === coach.therapistName)
                      const lastDate = staff && lastPayrollDates[staff.id]
                      return (
                        <div className="mt-2">
                          <button
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full">
            ...
            <div className="p-6 space-y-4">
              <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-xl p-4">
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">{t('physiotherapy.commission.commissionLabel')}</p>
                <p className="text-2xl font-bold text-green-600 dark:text-green-400">
                  {payrollCommission.toLocaleString(localeString, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {t('physiotherapy.commission.currency')}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">{t('physiotherapy.commission.salaryLabel')}</label>
                <input
                  type="number"
                  value={payrollSalary}
              ...
              {/* الخصومات المعلقة */}
              {loadingDeductions ? (
                <div className="text-center text-sm text-gray-400 dark:text-gray-500 py-2">{t('physiotherapy.commission.loadingDeductions')}</div>
                ...
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-xl p-4">
                  ...
                  {payrollDeductions.map(d => (
                    <div key={d.id} className="flex justify-between text-sm text-gray-700 dark:text-gray-300">
                      <span>{d.reason}</span>
                      <span className="font-bold text-red-600 dark:text-red-400">- {d.amount.toLocaleString(localeString)} {t('physiotherapy.commission.currency')}</span>
                    </div>
                  ))}
                  <div className="border-t border-red-200 dark:border-red-700 mt-2 pt-2 flex justify-between font-bold text-sm">
                    <span className="text-red-700 dark:text-red-300">{t('physiotherapy.commission.totalDeductionsLabel')}</span>
                    <span className="text-red-600 dark:text-red-400">- {payrollDeductions.reduce((s, d) => s + d.amount, 0).toLocaleString(localeString)} {t('physiotherapy.commission.currency')}</span>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-[9999]">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              ...
              {/* حدود الدخل + النسب — Flex tiers (2-5) */}
              {calculationMethod === 'revenue' && !useSeparateCoachTarget && (() => {
                const cs = commissionSettings
                const n = Math.min(5, Math.max(2, cs.tierCount || 5))
                ...
                const tierStyles = [
                  { bg: 'bg-green-50 dark:bg-green-900/50', border: 'border-green-200 dark:border-green-700', text: 'text-green-700 dark:text-green-300', inputBorder: 'border-green-300 dark:border-green-600', focus: 'focus:border-green-500 focus:ring-green-200', accent: 'text-green-600 dark:text-green-400' },
                  { bg: 'bg-blue-50 dark:bg-blue-900/50', border: 'border-blue-200 dark:border-blue-700', text: 'text-blue-700 dark:text-blue-300', inputBorder: 'border-blue-300 dark:border-blue-600', focus: 'focus:border-blue-500 focus:ring-blue-200', accent: 'text-blue-600 dark:text-blue-400' },
                  { bg: 'bg-yellow-50 dark:bg-yellow-900/50', border: 'border-yellow-200 dark:border-yellow-700', text: 'text-yellow-700 dark:text-yellow-300', inputBorder: 'border-yellow-300 dark:border-yellow-600', focus: 'focus:border-yellow-500 focus:ring-yellow-200', accent: 'text-yellow-600 dark:text-yellow-400' },
                  { bg: 'bg-orange-50 dark:bg-orange-900/50', border: 'border-orange-200 dark:border-orange-700', text: 'text-orange-700 dark:text-orange-300', inputBorder: 'border-orange-300 dark:border-orange-600', focus: 'focus:border-orange-500 focus:ring-orange-200', accent: 'text-orange-600 dark:text-orange-400' },
