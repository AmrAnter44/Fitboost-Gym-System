...
export default function PhysiotherapyPage({ embedded }: { embedded?: boolean } = {}) {
  ...
  const [renewalSession, setRenewalSession] = useState<PhysiotherapySession | null>(null)
  ...
  if (permissionsLoading) {
    return (
      <LoadingScreen fullScreen message={t('physiotherapy.loading')} />
    )
  }
    ...
    <div className="container mx-auto p-4 sm:p-6" dir={direction}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto p-6" dir={direction}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  return therapists.length === 0 ? (
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-amber-600 dark:text-amber-400">
                         {t('physiotherapy.noActivePhysiotherapyists')}
                      </p>
                    </div>
                  ) : (
                    <select
              ...
              {!editingSession && (
                <div className="bg-blue-50 dark:bg-blue-900/50 ring-1 ring-blue-200 dark:ring-blue-700 rounded-lg p-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {packages.map((pkg) => (
                      <button
                        ...
                        className="bg-gradient-to-br from-blue-50 to-cyan-100 dark:from-blue-900/50 dark:to-cyan-900/50 hover:from-blue-100 hover:to-cyan-200 dark:hover:from-blue-800/50 dark:hover:to-cyan-800/50 ring-1 ring-blue-400 dark:ring-blue-700 rounded-lg p-3 transition-colors duration-200 hover:shadow-lg dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                      >
            ...
            {formData.sessionsPurchased > 0 && formData.totalPrice > 0 && (
              <div className="bg-blue-50 dark:bg-blue-900/30 ring-1 ring-blue-200 dark:ring-blue-700 rounded-lg p-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6" dir={direction}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          {/* فلتر النوع (Physiotherapy عادي / Day Use) */}
          <div>
            <label className="block text-sm font-medium mb-1.5 text-gray-900 dark:text-gray-100">{t('physiotherapy.sessionType')}</label>
            <select
              value={filterType}
              onChange={(e) => setFilterType(e.target.value as any)}
              className="w-full px-3 py-2 ring-1 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white"
            >
              <option value="all">{t('physiotherapy.typeAll')}</option>
              <option value="regular">{t('physiotherapy.typeRegular')}</option>
              <option value="dayuse">{t('physiotherapy.typeDayUse')}</option>
            </select>
          </div>
        </div>
      ...
      {loading ? (
        <LoadingScreen message={t('physiotherapy.loading')} />
      ) : (
        <>
          {/* Cards Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" dir={direction}>
            {filteredSessions.map((session) => {
              ...
              return (
                ...
                >
                  {/* Header */}
                  <div className={`p-3 ${isExpired ? 'bg-red-600 dark:bg-red-700' : isExpiringSoon ? 'bg-orange-600 dark:bg-orange-700' : 'bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-800'}`}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-white/80 text-xs">
                            {session.physioNumber < 0 ? ' Day Use' : `#${session.physioNumber}`} • {session.phone}
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="p-3 space-y-2.5">
                    ...
                    <div>
                      <div className="flex justify-between text-xs text-gray-600 dark:text-gray-300 mb-1">
                        <span>{t('physiotherapy.therapist')}: {session.therapistName}</span>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-lg p-2 text-center">
                        <div className="text-[10px] text-blue-700 dark:text-blue-300 font-semibold">{t('physiotherapy.total')}</div>
                      ...
                      <div className={`border rounded-lg p-2 text-center ${
                        ...
                        <div className={`text-[10px] font-semibold ${(session.remainingAmount || 0) > 0 ? 'text-orange-700 dark:text-orange-300' : 'text-gray-500 dark:text-gray-400'}`}>{t('physiotherapy.remaining')}</div>
                        <div className={`text-sm font-bold ${(session.remainingAmount || 0) > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-blue-600 dark:text-blue-400'}`}>
                    ...
                    {(session.startDate || session.expiryDate) && (
                      <div className={`border rounded-lg p-2 text-xs font-mono ${
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isExpired && <span className="text-red-600 dark:text-red-400 font-bold">({t('physiotherapy.expired')})</span>}
                          {!isExpired && isExpiringSoon && <span className="text-orange-600 dark:text-orange-400 font-bold">({t('physiotherapy.expiringSoon')})</span>}
                        </div>
                      </div>
                    )}
                    ...
                    {!isCoach && (
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        {session.physioNumber >= 0 && (
                          <>
                            ...
                            >
                              ...
                              <span>{t('physiotherapy.attendance')}</span>
                            </button>
                            <button
                            ...
                            >
                              ...
                              <span>{t('physiotherapy.renew')}</span>
                            </button>
                          </>
                        )}
                        {(session.remainingAmount || 0) > 0 && (
                          ...
                          >
                            ...
                            <span>{t('physiotherapy.payRemaining')} ({(session.remainingAmount || 0).toFixed(0)} {t('physiotherapy.egp')})</span>
                          </button>
                        )}
                        <button
                        ...
                        <button
                          ...
                          aria-label={t('physiotherapy.deleteSubscription')}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6" dir={direction}>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-orange-50 dark:bg-orange-900/20 ring-1 ring-orange-200 dark:ring-orange-800/60 rounded-lg p-4">
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-400">{t('physiotherapy.physioNumber')}:</span>
                    <span className="font-bold text-gray-900 dark:text-gray-100">#{paymentSession.physioNumber}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-400">{t('physiotherapy.client')}:</span>
                    <span className="font-bold text-gray-900 dark:text-gray-100">{paymentSession.clientName}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-400">{t('physiotherapy.therapist')}:</span>
                    <span className="font-bold text-gray-900 dark:text-gray-100">{paymentSession.therapistName}</span>
                  </div>
                  <div className="flex justify-between border-t border-orange-200 dark:border-orange-800/60 pt-2">
                    <span className="text-orange-700 dark:text-orange-300 font-semibold">{t('physiotherapy.paymentModal.remainingAmount')}</span>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    ...
                    className="flex-1 px-3 py-1 bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-900/50 text-blue-800 dark:text-blue-200 rounded text-sm font-medium transition-colors duration-200"
                  >
              ...
              {paymentFormData.paymentAmount > 0 && (
                <div className="bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-200 dark:ring-blue-800/60 rounded-lg p-3">
                  <div className="flex justify-between items-center">
