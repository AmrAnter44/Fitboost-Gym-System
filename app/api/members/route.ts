...
export async function GET(request: Request) {
  try {
    ...
    const user = await requirePermission(request, 'canViewMembers')

    // ✅ السيلز بيشوف كل الأعضاء لو عنده صلاحية canViewMembers
    // الفلترة حسب salesStaffId بتتم في صفحة المتابعات فقط
    const salesOnlyFilter = {}
...
export async function POST(request: Request) {
  try {
    ...
    // 🔒 Zod validation - حدود وأنواع صارمة للبيانات المدخلة
    const parseResult = memberCreateSchema.safeParse(rawBody)
    if (!parseResult.success) {
      return NextResponse.json(
        { error: `بيانات غير صحيحة: ${formatZodError(parseResult.error)}` },
        { status: 400 }
      )
    }
...
export async function PUT(request: Request) {
  try {
    ...
    if (data.remainingAmount !== undefined) {
      ...
      if (parseInt(data.remainingAmount.toString()) === 0) {
        updateData.remainingDueDate = null
      }
    }
    if (data.remainingDueDate !== undefined) {
      updateData.remainingDueDate = data.remainingDueDate ? new Date(data.remainingDueDate) : null
    }
    ...
    if (data.startDate || data.expiryDate) {
      ...
      if (currentMember) {
        ...
        // ✅ نتحقق من الانتهاء فقط (مش البداية) — لأن العضو اللي اشتراكه لسه ما بدأش
        // هو مش منتهي، والـ client بيعرض "يبدأ بعد X يوم" باستخدام startDate
        const notExpired = !newExpiry || new Date(newExpiry) >= today
        updateData.isActive = notExpired
      }
    }
