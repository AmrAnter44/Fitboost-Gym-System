...
export async function POST(request: Request) {
  try {
    // ✅ إنشاء GroupClass متاح لأي موظف مسجّل دخول (مش الكوتش)
    ...
    // ✅ التحقق من رقم GroupClass يتم داخل الـ Transaction لمنع Race Condition
      ...
      const groupClass = await prisma.$transaction(async (tx) => {
        ...
        if (classNumber && parseInt(classNumber) > 0) {
          ...
          if (existingGroupClass) {
            throw new Error(`رقم GroupClass ${classNumber} مستخدم بالفعل`)
          }
        }
