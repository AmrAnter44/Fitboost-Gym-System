...
export async function POST(request: Request) {
  try {
    // ✅ إنشاء Physiotherapy متاح لأي موظف مسجّل دخول (مش الكوتش)
    ...
    // ✅ التحقق من رقم Physiotherapy يتم داخل الـ Transaction لمنع Race Condition
    ...
    const physioReferralEnabled = systemSettings?.physioReferralEnabled ?? false
    const physioReferralPercentage = systemSettings?.physioReferralPercentage ?? 0
    ...
    try {
      ...
      const physiotherapy = await prisma.$transaction(async (tx) => {
        // ✅ التحقق من رقم Physiotherapy داخل الـ Transaction لمنع Race Condition
        if (physioNumber && parseInt(physioNumber) > 0) {
          const existingPhysiotherapy = await tx.physiotherapy.findUnique({
            where: { physioNumber: parseInt(physioNumber) }
          })
          if (existingPhysiotherapy) {
            throw new Error(`رقم Physiotherapy ${physioNumber} مستخدم بالفعل`)
          }
        }
        ...
        if (physioReferralEnabled && referralCoachId && totalAmount > 0) {
          try {
            const commissionAmount = (totalAmount * physioReferralPercentage) / 100
            ...
            if (commissionAmount > 0) {
              await tx.commission.create({
                data: {
                  ...
                  description: `عمولة Referral علاج طبيعي - ${clientName} - ${physioReferralPercentage}% من ${totalAmount.toFixed(2)} ج.م (#${physiotherapy.physioNumber})`
                }
              })
            }
          } catch (referralError) {
