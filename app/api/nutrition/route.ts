...
export async function POST(request: Request) {
  try {
    // ✅ إنشاء Nutrition متاح لأي موظف مسجّل دخول (مش الكوتش)
    ...
    // ✅ التحقق من رقم Nutrition يتم داخل الـ Transaction لمنع Race Condition
    ...
    try {
      ...
      const nutrition = await prisma.$transaction(async (tx) => {
        // ✅ التحقق من رقم Nutrition داخل الـ Transaction لمنع Race Condition
        if (nutritionNumber && parseInt(nutritionNumber) > 0) {
          const existingNutrition = await tx.nutrition.findUnique({
            where: { nutritionNumber: parseInt(nutritionNumber) }
          })
          if (existingNutrition) {
            throw new Error(`رقم Nutrition ${nutritionNumber} مستخدم بالفعل`)
          }
        }
