...
export default function ReceiptWhatsApp({ receipt, onDetailsClick }: ReceiptWhatsAppProps) {
  ...
  const prepareReceiptMessage = (data: any) => {
    ...
    // تحويل نوع الإيصال إلى اسم عربي - نفس الماب الموجود في ReceiptToPrint
    ...
    if (details.upgradeAmount > 0) {
      message += `- سعر الترقية الكامل: ${details.upgradeAmount} ج.م\n`;
    }
    ...
    // عرض روابط التطبيق إذا كانت مفعلة
    if (showAppLinks && (androidAppUrl || iosAppUrl)) {
      message += `\n📱 *حمّل تطبيقنا:*\n`;
      if (androidAppUrl) {
        message += `🤖 Android:\n${androidAppUrl}\n`;
      }
