...
//  لوحة توزيع الليدز الكاستوم — تختار الداتا (مصدر + فترة + جندر + عدد) وتوزّعها على السيلز بنسب.
//  بتُستخدم في تاب «إدارة السيلز» جوّه المتابعات، وفي صفحة /sales-management (نفس الكود، بدون تكرار).
...
export default function SalesDistributionPanel({ showHeader = true }: { showHeader?: boolean }) {
  const { user, isAdmin, permissions } = usePermissions()
  ...
  const confirmDistribute = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchPreview()
        fetchStats()
        //  خبّر باقي لوحات السيلز (CollectionDashboard / SalesMgmtPanel) تعيد الحساب
  ...
  if (!canAccess) {
    return (
      <p className="text-sm text-gray-500 dark:text-gray-400 py-4">
        {ar ? 'ليس لديك صلاحية توزيع الليدز' : 'You do not have permission to distribute leads'}
      </p>
    )
  }
          ...
          <div className="space-y-2">
            {sortedSalesStaff.map(s => {
              ...
              return (
                <div
                  key={s.id}
                  onClick={() => toggleRep(s.id)}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleRep(s.id) } }}
                  aria-pressed={row.selected}
                  className={`flex items-center gap-3 rounded-lg px-3 py-2.5 ring-1 cursor-pointer transition-all duration-150 ${
                    row.selected
                      ? 'bg-primary-50 dark:bg-primary-900/20 ring-2 ring-primary-400 dark:ring-primary-600 shadow-sm'
                      : 'ring-gray-200 dark:ring-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/40 hover:ring-primary-300 dark:hover:ring-primary-700'
                  }`}
                >
                  {/*  علامة الاختيار (بدل الشيك بوكس) */}
                  <span className={`w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 ring-1 transition-colors ${
                    row.selected ? 'bg-primary-600 ring-primary-600 text-white' : 'ring-gray-300 dark:ring-gray-600 text-transparent'
                  }`}>
