...
export function ReceiptDetailModal({ receipt, onClose }: ReceiptDetailModalProps) {
  ...
  const getTypeColor = (type: string) => {
    const colors: { [key: string]: string } = {
      'Member': 'from-primary-500 to-primary-600 dark:from-primary-700 dark:to-primary-800',
      'PT': 'from-green-500 to-green-600 dark:from-green-700 dark:to-green-800',
      'DayUse': 'from-primary-500 to-primary-600 dark:from-primary-700 dark:to-primary-800',
      'InBody': 'from-orange-500 to-orange-600 dark:from-orange-700 dark:to-orange-800'
    }
    return colors[type] || 'from-gray-500 to-gray-600 dark:from-gray-700 dark:to-gray-800'
  }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={onClose} dir={direction}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-5xl w-full shadow-2xl max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className={`bg-gradient-to-r ${getTypeColor(receipt.type)} text-white p-4 rounded-t-2xl`}>
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={onClose}
              className="text-white hover:bg-white hover:bg-opacity-20 dark:hover:bg-black dark:hover:bg-opacity-20 rounded-full w-9 h-9 flex items-center justify-center transition text-2xl"
            >
              ×
            </button>
          </div>
        ...
        <div className="p-4 space-y-3 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            {details.memberName && (
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                <p className="text-xs text-gray-600 dark:text-gray-300 mb-1">{t('receipts.detail.memberName')}</p>
            ...
            {details.clientName && (
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                <p className="text-xs text-gray-600 dark:text-gray-300 mb-1">{t('receipts.detail.clientName')}</p>
            ...
            {details.name && (
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                <p className="text-xs text-gray-600 dark:text-gray-300 mb-1">{t('receipts.detail.name')}</p>
              ...
              <div className="space-y-1">
                {details.paidAmount !== undefined && (
                  <div className="flex justify-between py-1 border-b dark:border-gray-600 text-sm">
                    ...
                    <span className="font-bold text-green-600 dark:text-green-400">{details.paidAmount} {t('common.currency')}</span>
                  </div>
                )}
                ...
                <div className="py-1 border-b dark:border-gray-600 text-sm">
                  ...
                  {isMultiPayment ? (
                    <div className="flex flex-wrap gap-1">
                      {paymentData.methods.map((pm, idx) => (
                        <span
                          key={idx}
                          className="inline-flex items-center gap-1 bg-primary-100 dark:bg-primary-900/50 text-primary-800 dark:text-primary-300 px-2 py-0.5 rounded-full text-xs font-bold"
                        >
                    ...
                    <span className="inline-flex items-center gap-1 bg-primary-100 dark:bg-primary-900/50 text-primary-800 dark:text-primary-300 px-2 py-1 rounded-full text-xs font-bold">
                      {getPaymentMethodLabel(paymentData.methods[0].method, language)}
                    </span>
                  )}
                </div>

                <div className="flex justify-between py-2 bg-green-50 dark:bg-green-900/30 px-2 rounded-lg mt-2">
                  <span className="font-bold text-gray-800 dark:text-gray-100 text-sm">{t('receipts.detail.total')}</span>
                  <span className="font-bold text-xl text-green-600 dark:text-green-400">{receipt.amount} {t('common.currency')}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Date */}
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
            <p className="text-xs text-gray-600 dark:text-gray-300 mb-1">{t('receipts.detail.issueDate')}</p>
        ...
        {/* Footer Actions */}
        <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-b-2xl flex gap-3">
          <button
            onClick={handlePrint}
