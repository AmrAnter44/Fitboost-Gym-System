...
export default function GroupClassRenewalForm({ session, onSuccess, onClose }: GroupClassRenewalFormProps) {
  ...
  const fetchCoaches = async () => {
    try {
      ...
      const activeInstructors = data.filter(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.startDate && formData.expiryDate) {
      ...
      if (end <= start) {
        toast.error(t('groupClass.renewal.dateError'))
        setLoading(false)
        return
      }
    }
    ...
    try {
      const response = await fetch('/api/group-classes/renew', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          classNumber: session.groupClassNumber,
          ...formData,
      ...
      if (response.ok) {
        toast.success(t('groupClass.renewal.successMessage'))
        ...
        toast.error(result.error || t('groupClass.renewal.failureMessage'))
      }
    } catch (error) {
      console.error(error)
      toast.error(t('groupClass.renewal.connectionError'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="groupclass-renewal-title"
    >
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg ring-1 ring-gray-200 dark:ring-gray-700 max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-primary-500 text-primary-contrast p-4 rounded-t-2xl">
          <div className="flex justify-between items-center">
            <div>
              <h2 id="groupclass-renewal-title" className="text-xl font-bold mb-1 flex items-center gap-2">
                <svg className="w-6 h-6" {...stroke}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
                </svg>
                <span>{t('groupClass.renewal.title')}</span>
              </h2>
              <p className="text-gray-800 text-sm">{t('groupClass.renewal.subtitle')}</p>
            </div>
            <button
              onClick={onClose}
              ...
              aria-label={t('groupClass.cancelButton')}
            >
        ...
        <div className="p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {packages.map(pkg => (
                  <button
                    ...
                    className="bg-white dark:bg-gray-800 hover:bg-primary-50 dark:hover:bg-primary-900/20 ring-1 ring-primary-300 dark:ring-primary-900/50 hover:ring-primary-500 rounded-xl p-3 transition-colors duration-200"
                  >
          ...
          <div className={`bg-primary-50 dark:bg-primary-900/20 ${direction === 'rtl' ? 'border-e-4' : 'border-s-4'} border-primary-500 dark:border-primary-700 p-3 rounded-lg mb-4`}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div>
                <p className="text-xs text-gray-600 dark:text-gray-400">{t('groupClass.renewal.classNumber')}</p>
                <p className="text-xl font-bold text-primary-600 dark:text-primary-400">#{session.groupClassNumber}</p>
              </div>
              <div>
                <p className="text-xs text-gray-600 dark:text-gray-400">{t('groupClass.renewal.clientName')}</p>
                <p className="text-base font-bold text-gray-900 dark:text-gray-100">{session.clientName}</p>
              </div>
              <div>
                <p className="text-xs text-gray-600 dark:text-gray-400">{t('groupClass.renewal.currentInstructor')}</p>
                <p className="text-base text-gray-900 dark:text-gray-100">{session.instructorName}</p>
              </div>
              <div>
                <p className="text-xs text-gray-600 dark:text-gray-400">{t('groupClass.renewal.currentSessionsRemaining')}</p>
            ...
            {(session.remainingAmount || 0) > 0 && (
              <div className="mt-2 pt-2 border-t border-primary-200 dark:border-primary-900/50">
            ...
            {session.expiryDate && (
              <div className="mt-2 pt-2 border-t border-primary-200 dark:border-primary-900/50">
                <p className="text-xs text-gray-600 dark:text-gray-400 inline-block">{t('groupClass.renewal.currentExpiryDate')}: </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm ring-1 ring-primary-200 dark:ring-primary-900/50 p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                  <label htmlFor="gc-renew-phone" className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">
                    {t('groupClass.phoneNumber')}
                  </label>
                  <input
                    id="gc-renew-phone"
                    type="tel"
                ...
                <div>
                  <label htmlFor="gc-renew-sessions" className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">
                    {t('groupClass.renewal.newSessionsCount')} <span className="text-red-600">*</span>
                  </label>
                  <input
                    id="gc-renew-sessions"
                    type="number"
                    required
                    min="1"
                ...
                <div>
                  <label htmlFor="gc-renew-instructor" className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">
                  ...
                  {coachesLoading ? (
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1">
                        ...
                        <span>{t('groupClass.noActiveInstructors')}</span>
                      </p>
                    </div>
                  ) : (
                    <select
                    ...
                    >
                      ...
                      {instructors.map((instructor) => (
                        <option key={instructor.id} value={instructor.name}>
                          {instructor.name} {instructor.phone && `(${instructor.phone})`}
                        </option>
                      ))}
                    </select>
                  )}
              ...
              <div className="mt-3">
                <div>
                  <label htmlFor="gc-renew-price" className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-primary-400 dark:border-primary-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-colors duration-200 font-bold"
                    placeholder="0.00"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm ring-1 ring-primary-200 dark:ring-primary-900/50 p-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    <label htmlFor="gc-renew-start" className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">
                      {t('groupClass.startDate')} <span className="text-red-600">*</span>
                    </label>
                    <input
                      id="gc-renew-start"
                      type="text"
                  ...
                  <div>
                    <label htmlFor="gc-renew-expiry" className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">
                      {t('groupClass.expiryDate')} <span className="text-red-600">*</span>
                    </label>
                    <input
                      id="gc-renew-expiry"
                      type="text"
                ...
                <div className="mb-3">
                  <p className="text-xs font-bold mb-2 text-gray-700 dark:text-gray-300">{t('groupClass.quickAdd')}</p>
                  ...
                  <div className="bg-primary-50 dark:bg-primary-900/20 ring-1 ring-primary-200 dark:ring-primary-900/50 rounded-lg p-2">
                    {duration > 0 ? (
                      ...
                      <p className="text-red-600 dark:text-red-400 flex items-center gap-2 text-xs">
                        ...
                        <span>{t('groupClass.renewal.dateError')}</span>
                      </p>
                    )}
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    <div className={`bg-primary-50 dark:bg-primary-900/20 ${direction === 'rtl' ? 'border-e-4' : 'border-s-4'} border-primary-400 dark:border-primary-700 p-2 rounded`}>
                      <p className="text-xs text-primary-800 dark:text-primary-300">
                        {t('groupClass.renewal.replacementWarning', {
                          sessionsRemaining: session.sessionsRemaining.toString()
                        })}
                      </p>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400">{t('groupClass.renewal.newSessionsLabel')}</span>
                      <span className="font-bold text-primary-600 dark:text-primary-400">{formData.sessionsPurchased} {t('groupClass.session')}</span>
                    </div>
                    <div className={`bg-primary-100 dark:bg-primary-900/20 ${direction === 'rtl' ? 'border-e-4' : 'border-s-4'} border-primary-500 dark:border-primary-700 p-2 rounded`}>
                      <div className="flex justify-between">
                        <span className="font-bold text-gray-900 dark:text-gray-100 text-sm">{t('groupClass.renewal.paidAmount')}</span>
                        <span className="font-bold text-primary-600 dark:text-primary-400 text-base">{formData.totalPrice} {t('groupClass.egp')}</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex gap-3">
              ...
              >
                {loading ? t('groupClass.renewal.renewing') : t('groupClass.renewal.renewButton')}
              </button>
              <button
                type="button"
                onClick={onClose}
