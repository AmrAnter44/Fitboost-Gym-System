// Downloads the cloudflared binary into ./resources so electron-builder can bundle
// it (extraResources) for the in-app Cloudflare Tunnel (Settings → تانل), and so
// local dev has it too. `resources/` is gitignored, so build machines run this.
//
...
// Skip the download if a non-trivial binary is already present (idempotent builds).
...
function download(url, dest) {
  return new Promise((resolve, reject) => {
    ...
    const go = (u, n) => {
      ...
      https
        .get(u, { headers: { 'User-Agent': 'fitboost-tunnel' } }, (res) => {
...
async function fetchWin() {
  if (alreadyHave('cloudflared.exe')) return console.log('✓ resources/cloudflared.exe (cached)');
  console.log('↓ cloudflared-windows-amd64.exe');
  await download(BASE + 'cloudflared-windows-amd64.exe', path.join(RES, 'cloudflared.exe'));
  console.log('✓ resources/cloudflared.exe');
}

async function fetchDarwin() {
  if (alreadyHave('cloudflared')) return console.log('✓ resources/cloudflared (cached)');
...
(async () => {
  ...
  if (jobs.has('mac')) { try { await fetchDarwin(); } catch (e) { console.warn('! darwin fetch skipped: ' + e.message); } }
  if (jobs.has('linux')) { try { await fetchLinux(); } catch (e) { console.warn('! linux fetch skipped: ' + e.message); } }
  console.log('done.');
})().catch((e) => {
  console.error(e);
  process.exit(1);
