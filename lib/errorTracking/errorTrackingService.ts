 ...
 */
// ✅ حد أقصى لعدد سجلات الأخطاء في SQLite — لمنع تضخم الداتابيز
...
async function logErrorToDatabase(data: any, supabaseId: string | null): Promise<boolean> {
  try {
    ...
    // ✅ تنضيف تلقائي: احتفظ بآخر MAX_ERROR_LOGS فقط
    if (!_cleanupRunning) {
      _cleanupRunning = true
      prisma.errorLog.count().then(async (count) => {
        if (count > MAX_ERROR_LOGS) {
          const cutoff = await prisma.errorLog.findMany({
      ...
      }).catch(() => {}).finally(() => { _cleanupRunning = false })
    }
...
export async function logError(input: ErrorLogInput): Promise<void> {
  try {
    ...
    // 3. Extract and sanitize stack trace (remove absolute paths / tokens)
    let stackTrace = input.stackTrace
    if (!stackTrace && input.error instanceof Error) {
      stackTrace = input.error.stack || null
    }
    const sanitizedStackTrace = sanitizeStackTrace(stackTrace)

    // 4. Prepare data object — نخزن النسخة المنظفة فقط
    const errorData = {
