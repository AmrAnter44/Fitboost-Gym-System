 ...
 * ✅ يطبّع expiryDate لـ local midnight لتجنب مشاكل timezone
 */
export function calculateRemainingDays(expiryDate: Date | string | null | undefined): number | null {
  if (!expiryDate) return null

  const expiry = typeof expiryDate === 'string' ? new Date(expiryDate) : new Date(expiryDate)
  expiry.setHours(0, 0, 0, 0) // تطبيع لـ local midnight
