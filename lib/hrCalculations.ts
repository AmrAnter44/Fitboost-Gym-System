...
export function getExpectedWorkingDays(
  ...
  year: number,
  month: number,
  cutoffDate?: Date // للشهر الحالي: نحسب حتى اليوم فقط بدل نهاية الشهر
): number {
  const startOfMonth = new Date(year, month - 1, 1)
  const endOfMonth = new Date(year, month, 0)

  // نهاية الفترة الفعلية: تاريخ القطع أو نهاية الشهر (أيهما أصغر)
  const effectiveEnd = cutoffDate && cutoffDate < endOfMonth ? cutoffDate : endOfMonth

  if (joinDate > effectiveEnd) return 0

  // تحديد يوم البداية: تاريخ الانضمام (لو في نفس الشهر) أو أول الشهر
  ...
  for (let day = startDay; day <= endDay; day++) {
    const date = new Date(year, month - 1, day)
    if (date.getDay() !== 5) { // استثناء الجمعة
      workingDays++
    }
  }
