...
class WhatsAppQueue {
  ...
  startWorker(sessionIndex) {
    ...
    console.log(`[Queue:${sessionIndex}] Worker started`);
  }
  ...
  async _processItem(session, item, sessionIndex) {
    ...
    if (result.success) {
      ...
      console.log(`[Queue:${sessionIndex}] Sent to ${item.phone} (delay: ${Math.round(delay / 1000)}s)`);
    } else {
  ...
  async _resetDailyCounts() {
    ...
    if (result.success) {
      console.log('[Queue] Daily message counts reset');
    }
  }
}
